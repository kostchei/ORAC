...
- [x] **P1 — Risk throttle.** `policy.py::risk_class(tool, args)` + a total `(reversibility × externality)` throttle table → `auto`/`notify`/`approve`. Broker `_decide` consults it; the `APPROVAL_REQUIRED` stub is gone. `fs_read`/`git.commit`/`repo.write_file` are `auto`; `git.push` is `approve` (parks). Unclassified tools fail closed (raise). *Open: flip `(HARD, EXTERNAL_PRIVATE)` to `notify` if push should not gate (one cell in `_THROTTLE`).*
